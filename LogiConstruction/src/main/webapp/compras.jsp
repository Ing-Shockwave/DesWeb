...
<main class="compras-main">
    ...
    <section class="compras-body">
        ...
        <div class="compras-card">
            ...
            <form action="CompraServlet" method="post" class="compras-form" id="formCompra">
                <input type="text" name="proveedor" placeholder="Proveedor" required minlength="2" maxlength="100" autocomplete="organization">

                <input type="text" name="producto" list="listaMateriales" placeholder="Producto o material" autocomplete="off" required minlength="2" maxlength="100">
                ...
                <datalist id="listaMateriales">
                    ...
                    <option value="Tubería PVC"></option>
                    <option value="Varilla de acero"></option>
                    <option value="Vidrio templado"></option>
                    <option value="Yeso"></option>
                </datalist>

                <input type="number" name="cantidad" placeholder="Cantidad" required min="1" max="1000000" step="1">
            ...
            <% if ("validacion".equals(request.getParameter("error"))) { %>
                <p class="compras-error">Revise los campos: proveedor y producto deben tener mínimo 2 caracteres, y la cantidad debe ser mayor a 0.</p>
            <% } else if (request.getParameter("error") != null) { %>
                <p class="compras-error">No se pudo registrar la compra. Verifique la conexión o los datos ingresados.</p>
            <% } %>

            <% if (request.getParameter("eliminado") != null) { %>
                <p class="compras-ok">Compra eliminada correctamente.</p>
            <% } %>
        </div>
        ...
        <div class="compras-card">
            <div class="compras-card-head">
                ...
                <span>DataTables: búsqueda, ordenamiento y paginación</span>
            </div>
...
$(document).ready(function () {
    ...
    $('#buscarCompras').on('keyup', function () {
...
formCompra.addEventListener('submit', function (event) {
    const proveedor = formCompra.proveedor.value.trim();
    const producto = formCompra.producto.value.trim();
    const cantidad = Number(formCompra.cantidad.value);

    if (proveedor.length < 2 || producto.length < 2 || !Number.isInteger(cantidad) || cantidad < 1) {
        event.preventDefault();
        Swal.fire({
            icon: 'warning',
            title: 'Datos incompletos',
            text: 'Ingrese proveedor, producto y una cantidad válida mayor a cero.',
            background: '#0f172a',
            color: '#ffffff',
